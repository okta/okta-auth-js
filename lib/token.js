...
function getStateFromCookie() {
  return cookies.getCookie(config.REDIRECT_STATE_COOKIE_NAME);
}
...
function convertOAuthParamsToQueryParams(oauthParams) {
  ...
  if (oauthParams.responseType.indexOf('id_token') !== -1 &&
    ...
    if (oauthParams.scopes && oauthParams.scopes.length) {
      oauthQueryParams.scope = oauthParams.scopes.join(' ');
    } else {
      delete oauthQueryParams.scope;
    }
...
function getToken(sdk, oauthOptions, options) {
  ...
  if (oauthOptions.responseType === 'token' && !oauthOptions.scopes) {
    delete oauthParams.scopes;
  }
...
module.exports = {
  ...
  getStateFromCookie: getStateFromCookie
};
