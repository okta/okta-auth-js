...
function getAuthServerUrl(sdk, authServer) {
  authServer = authServer || sdk.options.authServer;
  if (authServer) {
    if (/^https?:/.test(authServer)) {
      return authServer;
    }
    return sdk.options.url + '/oauth2/' + authServer;
  }
}

var isAuthorizationServer = new RegExp('^https?://.*?/oauth2/.+');
function issuerToAuthorizationUrl(issuer) {
  if (isAuthorizationServer.test(issuer)) {
    return issuer;
  }
  return issuer + '/oauth2';
}

function getIssuer(sdk, authServer) {
  var authServerUrl = getAuthServerUrl(sdk, authServer);
  if (!authServerUrl) {
    return sdk.options.url;
  }
  if (isAuthorizationServer.test(authServerUrl)) {
    return authServerUrl;
  }
  return authServerUrl.substring(0, authServerUrl.indexOf('/oauth2'));
}

function validateClaims(sdk, claims, aud, issuer) {
  var iss = issuer || getIssuer(sdk);
...
function handleOAuthResponse(sdk, oauthParams, res, issuer) {
  ...
  if (res['id_token']) {
    ...
    validateClaims(sdk, jwt.payload, clientId, issuer);
...
function convertOAuthParamsToQueryParams(oauthParams) {
  ...
  } else if (oauthParams.scopes && oauthParams.scopes.length) {
...
function buildAuthorizeUrl(sdk, oauthParams, authServer) {
  var oauthQueryParams = convertOAuthParamsToQueryParams(oauthParams);
  var issuer = getIssuer(sdk, authServer);
  return issuerToAuthorizationUrl(issuer) + '/v1/authorize' +
    util.toQueryParams(oauthQueryParams);
}

/*
 * Retrieve a token from Okta or a third party idp
 * 
 * Two main flows:
 *
 ...
 *      scopes: defaults to ['email'] (with 'openid' if an id_token is requested)
 *
 *    Forced:
 *      prompt: 'none'
 *      responseMode: 'okta_post_message'
 ...
 * @param {String[]} [oauthOptions.scopes] OAuth 2.0 scopes to request (openid must be specified for ID Tokens)
...
function getToken(sdk, oauthOptions, options) {
  ...
  var requestUrl;
  try {
    requestUrl = buildAuthorizeUrl(sdk, oauthParams, options.authServer);
  } catch (e) {
    return Q.reject(e);
  }
  ...
  var issuer = getIssuer(sdk, options.authServer);
  ...
  switch (flowType) {
    case 'IFRAME':
      ...
      return iframePromise
        .then(function(res) {
          return handleOAuthResponse(sdk, oauthParams, res, issuer);
        })
        .fin(function() {
...
function getWithRedirect(sdk, oauthOptions, options) {
  ...
  var requestUrl = buildAuthorizeUrl(sdk, oauthParams, options.authServer);
  ...
  cookies.setCookie(config.REDIRECT_OAUTH_PARAMS_COOKIE_NAME, JSON.stringify({
    ...
    issuer: getIssuer(sdk, options.authServer)
  }));
...
function refreshToken(sdk, token) {
  if (!util.isToken(token)) {
    return Q.reject(new AuthSdkError(util.isTokenErrorMessage));
  }
  ...
  return sdk.token.getWithoutPrompt({
    ...
    authServer: issuerToAuthorizationUrl(token.issuer)
  });
}
...
function parseFromUrl(sdk, url) {
  ...
  return Q.resolve(hashToObject(hash))
    .then(function(res) {
      var issuer = oauthParams.issuer;
      delete oauthParams.issuer;
      return handleOAuthResponse(sdk, oauthParams, res, issuer);
    });
}
...
function getUserInfo(sdk, accessTokenObject) {
  if (!accessTokenObject ||
      (!util.isToken(accessTokenObject) && !accessTokenObject.accessToken)) {
    return Q.reject(new AuthSdkError('getUserInfo requires an access token object'));
  }

  var authorizationUrl = issuerToAuthorizationUrl(accessTokenObject.issuer);
  return http.httpRequest(sdk, {
    url: authorizationUrl + '/v1/userinfo',
    method: 'GET',
