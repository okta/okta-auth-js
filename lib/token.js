...
function exchangeCodeForToken(sdk, oauthParams, authorizationCode, urls) {
  ...
  return pkce.getToken(sdk, clonedParams, urls)
  .then(function(res) {
    validateResponse(res, clonedParams);
    return res;
  })
  .fin(function() {
    pkce.clearMeta(sdk);
...
function handleOAuthResponse(sdk, oauthParams, res, urls) {
  ...
  .then(function() {
    ...
    if (res['code']) {
      tokenTypes = pkce.loadMeta(sdk).responseType;
  ...
  .then(function(tokenDict) {
    ...
    validateTokenTypes.filter(function(key) {
      return (tokenTypes.indexOf(key) !== -1);
...
function getDefaultOAuthParams(sdk, oauthOptions) {
  ...
  var defaults = {
    grantType: sdk.options.grantType || 'implicit',
    clientId: sdk.options.clientId,
...
function prepareOauthParams(sdk, oauthOptions) {
  ...
  if (responseType.includes('code')) {
    throw new AuthSdkError('To use PKCE flow, set grantType: "authorization_code"');
  }
