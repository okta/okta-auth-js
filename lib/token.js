...
function getOAuthUrls(sdk, oauthParams, options) {
  ...
  // If an issuer exists, but it's not a url
  ...
  var isSharedResourceServer = new RegExp('^https?://.*?/oauth2/.+');

  // If issuer is supplied, but not authorizeUrl
  if (issuer && !authorizeUrl) {
    // Build authorizeUrl until we create it dynamically
    if (isSharedResourceServer.test(issuer)) {
  ...
  // If issuer is supplied, but not userinfoUrl
  if (issuer && !userinfoUrl) {
    // Build userinfoUrl until we create it dynamically
    if (isSharedResourceServer.test(issuer)) {
  ...
  // Anything not set should be defaulted
...
function handleOAuthResponse(sdk, oauthParams, res, urls) {
  ...
  if (res['id_token']) {
    ...
    validateClaims(sdk, jwt.payload, clientId, urls.issuer);
...
function getToken(sdk, oauthOptions, options) {
  ...
  try {
    ...
    urls = getOAuthUrls(sdk, oauthParams, options);
...
function getWithRedirect(sdk, oauthOptions, options) {
  ...
  var urls = getOAuthUrls(sdk, oauthParams, options);
