...
const isSameAuthState = (prevState: AuthState, state: AuthState) => {
  return (
    prevState.isPending === state.isPending &&
    prevState.isAuthenticated === state.isAuthenticated &&
    JSON.stringify(prevState.idToken) === JSON.stringify(state.idToken) &&
    JSON.stringify(prevState.accessToken) ===
...
export class AuthStateManager {
  ...
  updateAuthState(): void {
    if (
      ...
      warn(
        'updateAuthState is an asynchronous method with no return, ' +
    ...
    const log = (status) => {
      ...
      getConsole().group(
        `OKTA-AUTH-JS:updateAuthState: Event:${event} Status:${status}`
      );
