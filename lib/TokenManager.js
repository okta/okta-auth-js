...
function setRefreshTimeout(sdk, tokenMgmtRef, storage, key, token) {
  // Wait a minimum of 0 milliseconds (the token is already expired)
  var refreshWait = Math.max(0, (token.expiresAt * 1000) - Date.now());
...
function TokenManager(sdk, options) {
  ...
  this.on = tokenMgmtRef.emitter.on;
  this.off = tokenMgmtRef.emitter.off;
