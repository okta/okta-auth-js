...
* `clearTokensAfterRedirect` - If `true` (default: `false`) a flag will be added to the sessionStorage instead of clearing the local tokens immediately. Calling `oktaAuth.start()` after logout redirect will check the flag and clear local tokens. This option can be used when work with `SecureRoute` component from Okta's downstream client SDKs to guarantee the local tokens can only be cleared after the Okta SSO session is fully killed.
