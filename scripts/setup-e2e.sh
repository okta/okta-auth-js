...
setup_sample_tests () {
  ...
  if [[ "${BACON_TASK}" == true ]]; then
    ...
    get_vault_secret_key repo_gh-okta-okta-auth-js/default trex_client_secret CLIENT_SECRET
    # get_terminus_secret "/default" trex_idx_sdk_org_api_key OKTA_API_KEY
    get_vault_secret_key repo_gh-okta-okta-auth-js/default trex_idx_sdk_org_api_key OKTA_API_KEY
  else
    if [ -n "$USE_OK_14" ]; then
      ...
      get_vault_secret_key repo_gh-okta-okta-auth-js/prod/idx prod_client_secret_new CLIENT_SECRET
      # get_terminus_secret "/prod/idx" prod_idx_sdk_org_api_key_new OKTA_API_KEY
      get_vault_secret_key repo_gh-okta-okta-auth-js/prod/idx prod_idx_sdk_org_api_key_new OKTA_API_KEY
      # get_terminus_secret "/prod/idx" prod_idx_idfirst_sdk_org_api_key OKTA_API_KEY_IDFIRST
      get_vault_secret_key repo_gh-okta-okta-auth-js/prod/idx prod_idx_idfirst_sdk_org_api_key OKTA_API_KEY_IDFIRST
    else
      ...
      get_vault_secret_key repo_gh-okta-okta-auth-js/prod/idx prod_client_secret CLIENT_SECRET
      # get_terminus_secret "/prod/idx" prod_idx_sdk_org_api_key OKTA_API_KEY
      get_vault_secret_key repo_gh-okta-okta-auth-js/prod/idx prod_idx_sdk_org_api_key OKTA_API_KEY
    fi
  fi
  finish_log_group $?
}
