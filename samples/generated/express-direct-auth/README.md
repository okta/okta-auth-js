...
By default the app server runs at `http://localhost:`.

## Commands

If running from the workspace directory, add the `--cwd` option: `yarn --cwd samples/express-direct-auth start`
...
```html
http://localhost:/?issuer=https%3A%2F%2Fabc.oktapreview.com%2Foauth2%2Fdefault
```
