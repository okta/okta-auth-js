...
The [IDX API](./idx.md) is built on the [Okta Identity Engine][]. The API allows applications to implement features which were not possible with the older `authn` API, such as multi-factor authentication without redirection to Okta. We strongly recommend that all new application deployments use the `IDX` API. Existing applications can migrate to IDX by following this guide.
...
Applications must use the [PKCE OAuth 2.0 flow](../README.md#pkce-oauth-20-flow). In addition to `issuer`, ou must specify a `clientId` and `redirectUri` in you config. (`pkce` is enabled by default). A minimal config looks like:
